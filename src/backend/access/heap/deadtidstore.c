 ...
 * run is in sorted order because we during vacuum we scan and prune the heap
 * in block order.
 *
 * So while performing the first vacuum pass it is also possible that we
 * encounter the same deadtid again which we already added in the previous
 * runs. But there is no way to identify whether that is already there in the
 * conveyor or not except for checking it in the conveyor belt it self.
 *
 * Since we know that the individual runs in the conveyor belt are sorted so we
 * maintain a cache over each run and pull a couple of pages in the cache from
 * each run and then merge them into a block-level cache.  The block-level
 * cache is a very small cache that only holds the offset for the current block
 * we are going to prune.
 *
...
DTS_InsertDeadtids(DTS_DeadTidState *deadtidstate, char *data, int datasize)
{
	...
	while(datasize > 0)
	{
		 ...
		 * the remaining size based on how much we have copied.
		 */
